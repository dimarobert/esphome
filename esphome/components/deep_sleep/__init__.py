    ...
    if CORE.is_bk72xx:
        ...
        if len(wakeup_pins) > 1:
            ...
            esp32.only_on_variant(
                ...
                msg_prefix="Multiple wakeup pins",
            )(wakeup_pins)
            # INVERT_WAKEUP is not possible with multiple pins because
            # the GPIO wakeup API requires a single level for all pins
            invert = WAKEUP_PIN_MODES["INVERT_WAKEUP"]
            for pin_conf in wakeup_pins:
                if pin_conf.get(CONF_WAKEUP_PIN_MODE) is invert:
                    raise cv.Invalid(
                        "INVERT_WAKEUP is not supported with multiple wakeup pins"
                    )
    elif (
...
async def to_code(config):
    ...
    if CONF_WAKEUP_PIN in config:
        ...
        if CORE.is_bk72xx or (CORE.is_esp32 and len(pins_as_list) > 1):
